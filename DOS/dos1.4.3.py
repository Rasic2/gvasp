...
class PlotDOS():
	...
	def plot(self,xlim=None,show=False,**atoms):
		"""实例化一个DOS对象
		1、可以指定x坐标范围
		2、指定是否将DOS分别绘制
		"""
		DOS(self.total_dos,self.atom_list,self.element,xlim=xlim,show=show,**atoms)

class DOS():
	def __init__(self,total_dos,atom_list,element,xlim=None,show=False,**atoms):
		"""
	...
	def __plot_tot(self):
		...
		for column in self.total_dos.columns.values:
			plt.plot(self.total_dos.index.values,list(self.total_dos[column].values),label=column)
		plt.legend(loc='best')
	...
	def __plot_plus_tot(self):
		...
		if self.orbital==None:
			plt.plot(plus_tot.index.values,plus_tot['up'].values)
			plt.plot(plus_tot.index.values,plus_tot['down'].values)
		else:
			for orbital in self.orbital:
				plt.plot(plus_tot.index.values,plus_tot['{}_up'.format(orbital)])
				plt.plot(plus_tot.index.values,plus_tot['{}_down'.format(orbital)])

	def __plot_atom_tot(self):
		"""给出单个原子的up、down图"""
		plt.plot(self.atom_list[0],list(self.atom_list[self.atom]['up'].values),label=self.element[self.atom]+'_up')
		plt.plot(self.atom_list[0],list(self.atom_list[self.atom]['down'].values),label=self.element[self.atom]+'_down')
		plt.legend(loc='best')
	...
	def __plot_orbital(self):
		...
		try:
			for orbital in self.orbital:
				plt.plot(self.atom_list[0],list(self.atom_list[self.atom][orbital+'_up'].values),label=self.element[self.atom]+'_'+orbital+'_up') #modified at 2019/03/19 添加显示元素功能
				plt.plot(self.atom_list[0],list(self.atom_list[self.atom][orbital+'_down'].values),label=self.element[self.atom]+'_'+orbital+'_down')
		except KeyError:
