...
class BaseTask(metaclass=abc.ABCMeta):
    ...
    def _generate_INCAR(self, vdw, sol, nelect, mag=False, hse=False, static=False):
        """
    ...
    def _generate_KPOINTS(self, gamma=False):
        """
        ...
        if gamma:
            ...
            self.kpoints = " ".join(list(map(str, KPOINTS.min_number(structure=self.structure))))

        with open("KPOINTS", "w") as f:
            f.write("AutoGenerated \n")
            f.write("0 \n")
            f.write("Gamma \n")
            f.write(f"{self.kpoints} \n")
            f.write("0 0 0 \n")
