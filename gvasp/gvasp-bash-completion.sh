...
_gvasp_submit_opt() {
    ...
    allopts_short=(-P -V -S -C -l)
    allopts_long=(--potential --vdw --sol --continuous --low)
    allopts="-P --potential -V --vdw -S --sol -C --continuous -l --low"

    for (( i=0; i<${#allopts_short[@]}; i++ ))
    do
...
_gvasp_submit_chg() {
    ...
    if [[ "${COMP_WORDS[@]}" =~ "-s" || "${COMP_WORDS[@]}" =~ "--sequential" ]]; then
        allopts_short=(-P -s -V -S -C -a -l)
        allopts_long=(--potential --sequential --vdw --sol --continuous --analysis --low)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous -a --analysis -l --low"
    else
        allopts_short=(-P -s -V -S -C -a)
        allopts_long=(--potential --sequential --vdw --sol --continuous --analysis)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous -a --analysis"
    fi
...
_gvasp_submit_wf() {
    ...
    if [[ "${COMP_WORDS[@]}" =~ "-s" || "${COMP_WORDS[@]}" =~ "--sequential" ]]; then
        allopts_short=(-P -s -V -S -C -l)
        allopts_long=(--potential --sequential --vdw --sol --continuous --low)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous -l --low"
    else
        allopts_short=(-P -s -V -S -C)
        allopts_long=(--potential --sequential --vdw --sol --continuous)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous"
    fi
...
_gvasp_submit_dos() {
    ...
    if [[ "${COMP_WORDS[@]}" =~ "-s" || "${COMP_WORDS[@]}" =~ "--sequential" ]]; then
        allopts_short=(-P -s -V -S -C -l -a)
        allopts_long=(--potential --sequential --vdw --sol --continuous --low --analysis)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous -l --low -a --analysis"
    else
        allopts_short=(-P -s -V -S -C)
        allopts_long=(--potential --sequential --vdw --sol --continuous)
        allopts="-P --potential -s --sequential -V --vdw -S --sol -C --continuous"
...
_gvasp_submit_neb() {
    ...
    allopts_short=(-P -V -S -ini -fni -i -m -c)
    allopts_long=(--potential --vdw --sol --ini_poscar --fni_poscar --images --method --cancel_check_overlap)
    allopts="-P --potential -V --vdw -S --sol -ini --ini_poscar -fni --fni_poscar -i --images -m --method -c --cancel_check_overlap"
    ...
    if [[ "$pre" =~ "-h" || "$pre" =~ "--help" || "$pre" =~ "-i" || "$pre" =~ "--images" ]]; then
        opts=""
    elif [[ "$pre" =~ "-P" || "$pre" =~ "--potential" ]]; then
        opts="PAW_LDA PAW_PBE PAW_PW91 USPP_LDA USPP_PW91"
    elif [[ "$pre" =~ "PAW" || "$pre" =~ "USPP" ]]; then
...
_gvasp_submit_normal() {
    ...
    allopts_short=(-P -V -S)
    allopts_long=(--potential --vdw --sol)
    allopts="-P --potential -V --vdw -S --sol"
