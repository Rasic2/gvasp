...
#功能：对于bandgap的计算，区分ISPIN=2 or ISPIN=1 两种情况，HOMO区分了spinup和spindown的大小，LUMO统一用的是spinup的值；默认占据分数大于0.01为占据带。
#用法：perl OUTCAR 数值(大于该数值才被视作占据带)
print "\n";
...
if(@ARGV>0) {
    ...
    $cutoff = 0.01;
    print "Usage 'bandgap.pl cut=__' where __ is the maximum occupancy to consider a band unoccupied. The default is 0.01\n";
    print " \n";
}
...
open (OUTCAR,$ARGV[0]) || die "Cannot open OUTCAR!";
...
if ($ispin == 2) {
    ...
    if ($spinupHOMO == $spindownHOMO) {
        ...
        print "The top of the valence band differs for spin_up and spin_down states\n";
        print " \n";
        if ($spinupHOMO > $spindownHOMO) {
            $top = $spinupHOMO;
            $topkx = $spinupHOMOkx;
        ...
        print "The top of the valence band is a $topdir state with energy $top eV and it occurs at the k-point $topkx, $topky, $topkz\n";
        print " \n";
        $gap = $spinupLUMO - $top;
        print "The bottom of the conduction band is $spinupLUMO eV and it occurs at the k-point of $spinupLUMOkx $spinupLUMOky $spinupLUMOkz in reciprocal coordinates\n";
        print "\n";
