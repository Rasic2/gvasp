...
def dist(base_IS,pos_arr_IS,pos_arr_FS):
	pos_diff=ne.evaluate('pos_arr_IS-pos_arr_FS')
	pos_diff_pbc=ne.evaluate('where(pos_diff>0.5,pos_diff-1,pos_diff)')
	pos_diff_pbc=ne.evaluate('where(pos_diff_pbc<-0.5,pos_diff_pbc+1,pos_diff_pbc)')
...
def split(elenum_arr,base_IS,pos_arr_IS,pos_arr_FS):
	...
	pos_arr_FS_sortZ_index=np.argsort(pos_arr_FS[:,2])[:ne.evaluate('sum(elenum_arr)')-ne.evaluate('sum(elenum_arr)')]
...
def sort_genetic(elements_ori,elenum_ori,length_ori,base_IS,pos_arr_IS,pos_arr_FS,loc_dict_FS):
	...
	NP=200;D=ne.evaluate('sum(elenum_arr)');Pc=0.7;Pm=0.3;G=300
...
def main():
	...
	head_IS,base_IS,elenum_IS,loc_dict_IS,pos_arr_IS=load('IS')
	head_FS,base_FS,elenum_FS,loc_dict_FS,pos_arr_FS=load('FS')
