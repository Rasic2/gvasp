...
:program:`GVasp` support optimization, band structure, density of states, potential energy surface as well as the neb images plot. Each plot task can use :code:`--save` or :code:`--show` to save or show the output figure. For the plot setting and data load, one need to use :code:`-j/--json` to specify one json file to handle it.
